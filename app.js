...
function moveTo(item, target, speed) {
  if (item.x < target.x) {
    item.x +=
      (Math.abs(item.x - target.x) /
        Math.sqrt(
          Math.pow(Math.abs(item.x - target.x), 2) +
            Math.pow(Math.abs(item.y - target.y), 2)
        )) *
      speed;
  }
  if (item.x > target.x) {
...
function slicer() {
  ...
  if (player.skill == "slice") {
    ...
    if (
      slicePath.start.x !== slicePath.end.x &&
      slicePath.start.y !== slicePath.end.y
    ) {
      player.x += xspeed;
      player.y += yspeed;
    }
    ...
    for (i = 0; i < distance; i += 10) {
      ...
      zombies.forEach(function(j, index) {
        ...
        if (
          Math.sqrt(Math.pow(j.x - tempX, 2) + Math.pow(j.y - tempY, 2)) -
            (40 + j.radius) <=
          0
        ) {
          state = true;
...
function animate() {
  if (player.life > 0) {
    ...
    c.fillStyle = "red";
    window.location.href = `./lost.html?level=${player.level}&score=${
      player.score
    }`;
  }
  ...
  if (player.cooldown > 0) {
    ...
    c.arc(
      player.x,
      player.y,
      (player.cooldown * player.radius) / 1000,
      0,
      2 * Math.PI
    );
    c.fill();
  ...
  zombies.forEach(function(i, index) {
    ...
    if (
      Math.sqrt(Math.pow(i.x - player.x, 2) + Math.pow(i.y - player.y, 2)) -
